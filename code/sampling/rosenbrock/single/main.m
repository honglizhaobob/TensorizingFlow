...
grid on;
%% Plot marginal with Legendre nodes
% if there is a coefficient TT saved, plot marginals
if isfile("./data/rosenfullrank.mat")
    clear; clc; rng('default');
    load("./data/rosenfullrank.mat")
end
%%
% check exists 
legendre_ord; d;
...
surf(L1*X,L2*Y,marginal_dm2dm1); shading interp;
...
xlim([-0.4*L1 0.3*L1]); ylim([-0.8*L2 0]);
%xlabel("$x_{9}$", 'Interpreter','latex');
%ylabel("$x_{10}$", 'Interpreter','latex');

view(2);
exportgraphics(fig,'./img/rosen_dm2dm1.jpg','Resolution',500);
%%
...
fig = figure('PaperUnits', 'inches', 'PaperSize', ...
    [8 4], 'PaperPosition', [0 0 8 4]);
surf(L2*X,L3*Y,marginal_dm1d); shading interp;
...
xlim([-1*L2 0.1*L2]); ylim([-1*L3 0*L3]);
%xlabel("$x_{9}$", 'Interpreter','latex');
%ylabel("$x_{10}$", 'Interpreter','latex');
...
view(2);
exportgraphics(fig,'./img/rosen_dm1d.jpg','Resolution',500);
%% Plot marginal on uniform grid
% if there is a coefficient TT saved, plot marginals
...
for i = 1:d
    % uniform grid
    nodes = linspace(inp(i,1),inp(i,2),5*inp(i,3));
    dx = nodes(2)-nodes(1);
    % trapz weights
    W = trapz_weights(5*inp(i,3), dx);
    legendre_grids{i} = nodes;
    legendre_weights{i} = W';
    % get legendre polynomial data 
    % on legendre grids
...
marginal_dm2dm1 = cell2core(tt_tensor(),...
    {marginalized_new{1};marginalized_new{2}});
marginal_dm2dm1 = full(marginal_dm2dm1,[5*inp(d-2,3) 5*inp(d-1,3)]);
% for visualization, build meshgrid
[Y,X] = meshgrid(legendre_grids{d-1},legendre_grids{d-2});
figure(1);
...
marginal_dm1d = cell2core(tt_tensor(),...
    {marginalized_new{2};marginalized_new{3}});
marginal_dm1d = full(marginal_dm1d,[5*inp(d-1,3) 5*inp(d,3)]);
% for visualization, build meshgrid
[Y,X] = meshgrid(legendre_grids{d},legendre_grids{d-1});
figure(2);
...
%%
save("./data/rosenbrock_sample.mat");
