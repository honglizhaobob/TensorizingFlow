    ...
    def init_parameters(self, near_identity=False):
        ...
        for param in self.parameters():
            if not near_identity:
                ...
                param.data.fill_(0)
            # case specific
            if near_identity == "double_rosen":
                param.data.uniform_(-0.25, 0.25)
        ...
        super(SpinGlassResidualFlow, self).__init__(dim=dim,\
             trace_series_truncation_level=1000, \
                 n_hidden=128, \
                     n_layers=3)
# GL2D 
class GL2DResidualFlow(NaiveResidualFlow):
    def __init__(self, dim):
        super(GL2DResidualFlow, self).__init__(dim=dim,\
             trace_series_truncation_level=1500, \
                 n_hidden=64, \
                     n_layers=5)
...
# Random starting point, double Rosenbrock
class DoubleRosenResidualFlow(NaiveResidualFlow):
    def __init__(self, dim):
        super(DoubleRosenResidualFlow, self).__init__(dim=dim,\
             trace_series_truncation_level=1000, \
                 n_hidden=150, \
                     n_layers=3)
        # override with more random initial weights
        self.init_parameters(near_identity='double_rosen')
...
RESNET_BLOCKS2_GL2D2 = [ GL2DResidualFlow, GL2DBatchNormFlow ]
...
RESNET_BLOCKS2_DROSEN2 = [ DoubleRosenResidualFlow, DoubleRosenBatchNormFlow ]
